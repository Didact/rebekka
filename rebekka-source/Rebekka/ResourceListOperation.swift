...
internal class ResourceListOperation: ReadStreamOperation {
    ...
    override func streamEventEnd(_ aStream: Stream) -> (Bool, NSError?) {
        ...
        var offset = 0
        var bytes: UnsafeMutablePointer<UInt8> = UnsafeMutablePointer<UInt8>(bitPattern: 0)!
        self.inputData!.withUnsafeMutableBytes {
            bytes = $0
        }
        let totalBytes = CFIndex(self.inputData!.count)
        var parsedBytes = CFIndex(0)
        let entity = UnsafeMutablePointer<Unmanaged<CFDictionary>?>.allocate(capacity: 1)
        var resources = [ResourceItem]()
        repeat {
            CFFTPCreateParsedResourceListing(nil, bytes, 2, entity)
        ...
        if let inputStream = aStream as? InputStream {
            ...
            if result > 0 {
                if self.inputData == nil {
                    self.inputData = Data(bytes: buffer[0..<result])
                } else {
                    self.inputData!.append(buffer, count: result)
                }
            }
        }
        return (true, nil)
